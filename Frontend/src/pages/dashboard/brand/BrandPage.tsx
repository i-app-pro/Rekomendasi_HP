...
export default function BrandPage() {
  ...
  const load = async () => {
    try {
      ...
      setError(getErrorMessage(err, 'Gagal memuat data brand dari server.'));
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      alert(getErrorMessage(err, 'Gagal menghapus brand.'));
    }
  };
  ...
  const handleCreateBrand = async (nama: string) => {
    await createBrand(nama);
    await load();
  };

  const handleUpdateBrand = async (id: number, nama: string) => {
    await updateBrand(id, nama);
    await load();
  };

  const displayData = [...brands];
  while (displayData.length < 5) {
    displayData.push({ id: -1 - displayData.length, nama: '' });
  }
  ...
  return (
    <div className="w-full flex flex-col gap-4">
            ...
            {brands.map((row) => (
              <div key={row.id} className="bg-white p-5 rounded-2xl shadow-sm border border-slate-200 flex flex-col gap-3">
                ...
                <div className="flex flex-col pt-1">
                  <span className="text-xl font-black text-slate-800">{row.nama}</span>
                </div>
              </div>
            ))}
      ...
      <CreateBrand isOpen={isCreateOpen} onClose={() => setIsCreateOpen(false)} onSave={handleCreateBrand} />
      <EditBrand isOpen={isEditOpen} onClose={() => setIsEditOpen(false)} onUpdate={handleUpdateBrand} brandData={selectedBrand} />
