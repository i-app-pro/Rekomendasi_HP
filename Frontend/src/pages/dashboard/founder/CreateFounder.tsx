...
export default function CreateFounder({ isOpen, onClose, onSave }: CreateFounderProps) {
  const [formData, setFormData] = useState<FounderData>({
    nama: '', status: 'Mahasiswa', universitas: '', keahlian: '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onSave(formData);
    setFormData({ nama: '', status: 'Mahasiswa', universitas: '', keahlian: '', username_ig: '', email: '', github: '' });
    onClose();
  };
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 flex flex-col gap-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              ...
              <input type="text" name="keahlian" value={formData.keahlian} onChange={handleChange} required className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500" placeholder="Contoh: Frontend Developer" />
            </div>
            <div>
            ...
            <div className="sm:col-span-2">
              <label className="block text-xs font-bold text-slate-700 mb-1">URL GitHub</label>
              <input type="text" name="github" value={formData.github} onChange={handleChange} className="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm outline-none focus:ring-2 focus:ring-blue-500" placeholder="https://github.com/username" />
            </div>
          </div>
