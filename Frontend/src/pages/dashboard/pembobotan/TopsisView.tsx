...
// Response TOPSIS membawa field tambahan "d_plus" & "d_minus" (jarak ke solusi ideal).
          ...
          <table className="w-full text-sm text-left border-separate" style={{ borderSpacing: '0 12px' }}>
            <thead className="text-sm text-white font-bold bg-[#1e2530]">
              <tr>
                ...
                <th className="px-6 py-4 text-center">D-</th>
                <th className="px-6 py-4 rounded-r-xl text-center">Skor TOPSIS</th>
              </tr>
            </thead>
            <tbody>
              {data.length === 0 && (
              ...
              {data.map((row) => (
                <tr key={row.product_id} className="bg-white text-slate-800 font-semibold shadow-sm border border-slate-100">
                  ...
                  <td className="px-6 py-4 border-y border-slate-200 text-center">{row.d_plus?.toFixed(3) ?? '-'}</td>
                  <td className="px-6 py-4 border-y border-slate-200 text-center">{row.d_minus?.toFixed(3) ?? '-'}</td>
