...
import { type UserData } from './CreateUser';

interface EditUserProps {
  isOpen: boolean;
  onClose: () => void;
  onUpdate: (data: UserData) => void;
  userData: UserData | null; // Menerima data user yang dipilih dari UserPage
}

export default function EditUser({ isOpen, onClose, onUpdate, userData }: EditUserProps) {
  const [formData, setFormData] = useState<UserData>({ name: '', email: '', password: '', role: 'User' });

  // Tiap kali modal edit dibuka dan membawa data baru, sinkronkan ke state form
  useEffect(() => {
    if (userData) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    onUpdate(formData); // Kirim data hasil update
    onClose();
  };
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl overflow-hidden">
        ...
        <form onSubmit={handleSubmit} className="p-6 flex flex-col gap-4">
          ...
          <div className="flex justify-end gap-3 mt-4 pt-4 border-t border-slate-100">
            ...
            <button type="submit" className="px-5 py-2 rounded-lg font-bold text-white bg-blue-600 hover:bg-blue-700 shadow-sm">Update</button>
          </div>
        </form>
      </div>
    </div>
